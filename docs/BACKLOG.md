...
## Prioridad Alta
- [ ] 🔴 **Cron de sincronizacion de LaLiga 2026/27** — No existe cron para `POST /api/sync`; los partidos de LaLiga (empiezan 2026-08-16) no se sincronizan ni se puntuan automaticamente, solo con el boton de sync manual. Al retirar el cron del Mundial el 2026-08-15 la app se quedo sin ninguna sincronizacion automatica. Definir ventana horaria (los partidos suelen caer entre 12:00 y 23:00 hora de Madrid) y crear el cron en el VPS reutilizando el patron del script retirado en `/root/betsoccer-sync-worldcup.sh.retired`, con `SYNC_API_SECRET` para autenticacion desatendida.
