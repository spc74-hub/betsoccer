...
- **feat(castellon):** la vista muestra escudos, la temporada completa plegable y avisa con **"hora por confirmar"** en los partidos cuyo horario LaLiga aun no ha fijado (38 de 42 al arrancar la temporada figuran con un relleno de domingo a las 18:00). Enseñar ese 18:00 como firme seria mentir.
- **fix(castellon):** **Marca escribe la hora de Madrid con sufijo `Z`**, que significa UTC. El partido del 23/08 figura como `19:00Z` cuando su hora UTC real es 17:00, contrastado con la API. Se interpreta como `Europe/Madrid` y se convierte; sin eso, **todos** los horarios saldrian dos horas tarde.
- **fix(castellon):** si Marca no responde o cambia el HTML, el parseo devuelve None y se sigue con el metodo antiguo de escanear fechas contra la API. Verificado con Marca caida, con y sin calendario cacheado: la pagina nunca se queda en blanco.

## 2026-08-23
- **feat(castellon):** nueva seccion `/castellon` con proximos partidos, ultimos resultados y clasificacion completa de LaLiga Hypermotion, **visible solo para el usuario administrador** y **totalmente separada de las apuestas**: no toca `matches`, `predictions` ni `points`, y no persiste partidos en la tabla de la competicion (su `external_id` es de football-data.org y mezclar fuentes provocaria colisiones en el calculo de puntos). Nuevos ficheros: `backend/app/services/castellon.py`, `backend/app/routers/castellon.py`, `src/app/(protected)/castellon/page.tsx`. Modificados: `backend/app/main.py`, `backend/app/config.py`, `backend/app/models/models.py` (tabla `api_cache`), `backend/app/schemas.py` y `backend/app/routers/auth.py` (`is_admin`), `src/components/Navbar.tsx`, `src/types/index.ts`.
- **feat(auth):** `UserOut` incorpora `is_admin`, calculado comparando el email con `ADMIN_EMAIL` igual que `require_admin` (la app sigue sin tabla de roles). Lo devuelven `/api/auth/login`, `/api/auth/cf-access` y `/api/auth/me`; el Navbar lo consulta para decidir si pinta el enlace de Castellon.
- **feat(cache):** nueva tabla `api_cache` (clave, payload JSONB, fecha) usada como cache de APIs externas y como contador mensual de consumo. Esta en BD a proposito: en memoria se perderia en cada reinicio y en disco en cada deploy, y con una cuota de 100 peticiones **al mes** perder la cache es perder cuota real.
- **fix(config/prod):** `ADMIN_EMAIL` **no estaba definida** en el `.env` del VPS, asi que `require_admin` comparaba contra el valor por defecto de `config.py` (`admin@example.com`) y **no coincidia con ningun usuario**. Efecto: la seccion del Castellon devolvia 403 al propio administrador y, sobre todo, **`POST /api/admin/recalculate-points` llevaba inservible en produccion desde que existe** — fallo preexistente que esta seccion saco a la luz. Anadida `ADMIN_EMAIL` al `.env` y recreado el contenedor (`up -d --force-recreate`: un `restart` no relee el `env_file`).
- **fix(castellon):** el contador de cuota se sincroniza con la cabecera `X-RateLimit-Requests-Remaining` de cada respuesta en vez de contar por su cuenta. La ventana de RapidAPI **no es el mes natural** (se renueva a los 31 dias de la suscripcion), asi que un contador propio que se reinicia el dia 1 habria creido tener cuota disponible y se habria comido 429 reales. Contar a mano queda solo como respaldo si la cabecera falta.
- **fix(laliga):** retirado el parametro `division=segunda` de `GET /api/laliga` y la constante `SEGUNDA: "SD"`. Era codigo muerto que el frontend nunca llamaba y que **habria devuelto 403 al primer clic**: Segunda Division no esta incluida en el plan gratuito de football-data.org (verificado el 2026-08-23 contra la key real de produccion).
