...
- [x] **El calendario costaba una llamada por fecha** — **✅ RESUELTO 2026-08-23:** ahora sale de la pagina publica de Marca en una sola peticion que no consume cuota de la API. Queda esta dependencia nueva: **es scraping de un tercero**. Se lee el bloque JSON-LD de schema.org (formato estandar, aguanta mejor un rediseño) y no la tabla HTML, y si el parseo falla se vuelve solo al metodo antiguo. Si algun dia Marca bloquea o cambia, revisar `services/marca_calendar.py`.
- [ ] **(historico) El calendario costaba una llamada por fecha** — Esta API no tiene endpoint de "partidos de un equipo". Se escanea la ventana [-10, +17] dias congelando las fechas ya jugadas (no vuelven a pedirse nunca). Si algun dia aparece un endpoint por equipo, el coste baja a 1 llamada y se pueden relajar los intervalos de `config.py`.
